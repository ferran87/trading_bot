...
settings.yaml  → CONFIG.settings
    ...
    fees:        {xetra_eur, lse_gbp, nyse_usd, ...}  (per-venue fee rates for mock backend)
...
---------------------------------------------------------------------
BROKER_BACKEND              mock | t212  (default: mock)
...
class Config:
    ...
    def db_url(self) -> str:
        ...
        Priority:
          1. DATABASE_URL_T212  — when BROKER_BACKEND=t212
          2. DATABASE_URL       — generic fallback (legacy / Streamlit Cloud single-secret)
          3. Local SQLite       — offline / development
